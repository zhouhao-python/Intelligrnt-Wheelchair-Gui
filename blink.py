...
class Worker(QThread):
    ...
    def __init__(self, cap_num,parent=None):
        ...
        self.EYE_EAR = 0.18 # EAR阈值 眨眼判定的幅度 越小，越容易判断
        self.use_cpu_dlib = False
                ...
                if dets:
                    for i, d in enumerate(dets):
                        ...
                        if len(self.time_list) == 3:
                            print("debug",self.time_list[-1]-self.time_list[-3])
                            if (self.time_list[-1]-self.time_list[-3]) > 1 and (self.time_list[-1]-self.time_list[-3]) < 5:
                                self.sig.emit(3)
                                self.stop_flag = True
                                self.time_list = []
                                            ...
                                            for i, d in enumerate(dets_stop):
                                                ...
                                                if ear_stop < self.EYE_EAR:  # 如果EAR小于阈值，开始累计连续眨眼次数
                                                    ...
                                                    if frame_counter_stop >= self.EYE_EAR_BEYOND_TIME:  # 连续帧计数超过EYE_EAR_BEYOND_TIME的帧数时，累加超时次数（blink_counter+1）并给予提示警告
                                                        blink_counter_stop += 1
                                                        self.time_list.append(time.time())
                                                        # print(self.time_list[-1])
                                                        # print("len",len(self.time_list))
                                                        # print("zhouhao")
                                                        # print("frame_counter stop", frame_counter_stop)
                                                        frame_counter_stop = 0
                                                        # print("blink_counter stop", blink_counter_stop)
                                                if len(self.time_list) == 3:
                                                    print(self.time_list[-1] - self.time_list[-3])
                                                    if (self.time_list[-1]-self.time_list[-3]) > 1 and (self.time_list[-1]-self.time_list[-3]) < 5:
                                                        self.time_list = []
                                                        self.stop_flag = False
                                                    self.time_list = []
    ...
    #                         print(self.time_list[-1]-self.time_list[-2])
    #                         if (self.time_list[-1] - self.time_list[-2]) > 1:
    #                             self.run()
    #                             self.stop_flag = False
    #                         self.time_list = []
